...
#pragma config(DatalogSeries, 0, "", Sensors, Sensor, dgtl1, 50)
#pragma config(DatalogSeries, 1, "", Sensors, Sensor, dgtl2, 50)
...
task usercontrol()
{
	...
	while (true)
	{
		...
		int clawerror = clawtarget - ((abs(clawpotenleftvalue) + abs(clawpotenrightvalue)) / 2);
		motor[clawleft] = clawerror /5;
		motor[clawright] = clawerror /5;
		if (clawerror > 127) {
			clawerror = 127;
		}
		if (clawerror < -127){
