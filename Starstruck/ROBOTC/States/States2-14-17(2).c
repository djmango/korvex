...
#pragma config(Motor,  port7,           clawright,     tmotorVex393TurboSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port8,           clawleft,      tmotorVex393TurboSpeed_MC29, openLoop)
...
//		wait1Msec(1000);
	while (true)
	{
		...
		if(SensorValue[clawrightencoder] < SensorValue[clawleftencoder] && clawsynced == false){
			clawleftvalue = 20;
			clawrightvalue = -20;
		}
		if(SensorValue[clawrightencoder] > SensorValue[clawleftencoder] && clawsynced == false){
		...
		displayLCDCenteredString(1, "great again");
