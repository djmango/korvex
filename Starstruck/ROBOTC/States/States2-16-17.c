...
task autonomous()
{
	...
	case 0:
		...
		rightinchestomove = 3;
		openclaw();
		while (nextstep == 2) //turn to grab cube
		{
			turnright();
		}
		inchreset();
		...
		inchreset();
		inchestomove = -15; //move back to fence for cube drop
		while(nextstep == 6)
		{
			movebackward();
		}
		...
		openclaw();
		inchestomove = 27;
		while(nextstep == 9) // go for stars again; failsafe
		{
			moveforward();
		}
		inchreset();
